...
import br.com.mobiclub.quantoprima.R.layout;
...
import br.com.mobiclub.quantoprima.ui.adapter.HeaderFooterListAdapter;
...
public abstract class
        ItemListFragment<E> extends Fragment
        implements LoaderCallbacks<List<E>>, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onActivityCreated(final Bundle savedInstanceState) {
        ...
        getLoaderManager().initLoader(Constants.Loader.LIST, getLoaderParams(), this);
    }
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        View view = inflater.inflate(layout.fragment_list, null);
        frameNoContent = (FrameLayout) view.findViewById(R.id.frame_no_content);
        frameList = (RelativeLayout) view.findViewById(R.id.frame_list);
        return view;
    }
    ...
    public void onViewCreated(final View view, final Bundle savedInstanceState) {
        ...
        swipeLayout.setColorSchemeResources(R.color.logo_mobi_red,
                R.color.logo_mobi_blue1,
                R.color.logo_mobi_blue2,
                R.color.logo_mobi_orange);

        swipeLayout.setEnabled(enableSwipe());
    ...
    public void onRefresh() {
        Ln.d("onRefresh");
        //stopSwipeRefreshing();
        getLoaderManager().restartLoader(Constants.Loader.LIST, getLoaderParams(), this);
    }
    ...
    public void onLoadFinished(final Loader<List<E>> loader, final List<E> items) {
        ...
        if (loader instanceof ThrowableLoader) {
            ((ThrowableLoader<List<E>>) loader).clearException();
        }
        if (exception != null) {
    ...
    protected void addItems(List<E> items) {
        if(!isAdded())
            return;

        ListAdapter listAdapter = getListAdapter();
        if(listAdapter instanceof HeaderViewListAdapter) {
            HeaderViewListAdapter hd = (HeaderViewListAdapter) listAdapter;
            listAdapter = hd.getWrappedAdapter();
        }
        if(items == null && listAdapter instanceof  SingleTypeAdapter) {
            ((SingleTypeAdapter)listAdapter).setItems(Collections.emptyList());
            Ln.e("items == null");
            return;
        }
        if(listAdapter != null && listAdapter instanceof  SingleTypeAdapter) {
            ((SingleTypeAdapter) listAdapter).setItems(items.toArray());
        }
    }
     ...
     * @return adapter
     */
    protected TypeAdapter createListAdapter() {
...
//                wrapped);
     ...
     * @return adapter
     */
    protected abstract TypeAdapter createAdapter(final List<E> items);
    /**
    ...
    /**
     * Get header list adapter
     *
     * @return list adapter
     */
    @SuppressWarnings("unchecked")
    protected HeaderFooterListAdapter<SingleTypeAdapter<E>> getHeaderListAdapter() {
        if (listView != null) {
    ...
    public void showNoContentFragment(int layout) {
        items = Collections.emptyList();
        FragmentManager fm = getChildFragmentManager();
        FragmentNoContent fragment = new FragmentNoContent();
        Bundle args = new Bundle();
        args.putInt(Constants.Extra.NO_CONTENT_LAYOUT, layout);
        fragment.setArguments(args);
        fm.beginTransaction().replace(R.id.frame_no_content, fragment).commit();
